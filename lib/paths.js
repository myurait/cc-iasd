...
// worktree 実体の配置先（out/<run-id>/wt/<repo>）。run open がここに worktree を張り、
// created.data.worktree.path に焼く（束1 契約 paths 章）。
export function worktreeDir(root, runId, repo) {
  return path.join(outDir(root), runId, 'wt', repo);
}
