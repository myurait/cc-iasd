...
// check の失敗理由を決定論的に導出する（閉集合 REASON = passed/unverified/refuted）。
// passed=exit 一致（内容も走行も期待どおり）。
// unverified=実行不能（spawn 失敗＝res.error present。ENOENT / timeout 等の環境要因）。
// refuted=exit 不一致（プロセスは走ったが期待 exit と不一致＝内容要因）。
// unverified / refuted はいずれも pass ではない（Default-FAIL 不変）。
// 純関数。res は spawnSync の戻り。
export function classifyCheck(res, exit, expectExit) {
  if (res.error != null) return 'unverified';
  if (exit === expectExit) return 'passed';
  return 'refuted';
}
...
export function runCheck(root, check) {
  ...
  const reason = classifyCheck(res, exit, expectExit);
  return {
    id: check.id,
    run: check.run,
    cwd: check.cwd || '.',
    ...
    reason, // 閉集合 passed/unverified/refuted。reason==='passed' ⇔ pass===true。
    stdout: res.stdout || '',
...
export function run(root, runId, opts = {}) {
  ...
  // verdict-level 集約: 失敗 check（pass=false）の reason のユニーク集合。
  // 全 check pass なら []。checksPass=false の主因が checks 側にあることを示す。
  const failureReasons = [...new Set(checkResults.filter((c) => !c.pass).map((c) => c.reason))];
