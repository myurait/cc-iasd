...
import { coverageMatrix } from '../covers.js';
...
// run の対象 spec を解決する。derive が run node に保持する run.spec（created の data.spec）を
// 一次とし、無ければ node.refs の upstream ref（run open が焼く spec:<id>）から導出する。
// spec 未紐付け（adhoc 等）なら null。
function runSpecOf(runInfo, node) {
  if (runInfo && runInfo.spec) {
    return String(runInfo.spec).startsWith('spec:') ? runInfo.spec : `spec:${runInfo.spec}`;
  }
  const upstream = refsByRel(node, 'upstream').filter((to) => to && to.startsWith('spec:'));
  return upstream.length ? upstream[0] : null;
}
...
function cmdStatus({ positional, flags, root, jsonMode }) {
  ...
  // --plan: 中期計画ビュー
  if (flags.plan) {
    return statusPlan({ snapshot, root, jsonMode });
  }
...
function statusRef({ events, snapshot, ref, nowMs, jsonMode }) {
  ...
  if (runInfo) {
    ...
    // spec は run node（derive が created の data.spec を保持）を一次とし、無ければ
    // upstream refs（buildOpenRefs が焼く spec:<id>）から導出する。
    const spec = runSpecOf(runInfo, node);
    out.run = {
      type: runInfo.type || 'normal',
      campaign: runInfo.campaign || null,
      spec,
      running,
      stale_minutes: mins,
    };
...
function emitRefView(out, jsonMode) {
  ...
  if (out.run) {
    ...
    if (r.campaign) s.push(`campaign=${r.campaign}`);
    if (r.spec) s.push(`spec=${r.spec}`);
...
// --plan: route=vision の routed/open gap（中期計画在庫）、capability カバレッジ、
// campaign の depends_on / coverage 順序を射影する。
function statusPlan({ snapshot, root, jsonMode }) {
  // capability カバレッジ（covers 射影。06 8 章）。vision の Capabilities を宣言正本とし、
  // spec / campaign の covers ref（journal 一次 / frontmatter 二次）で突合する。
  const coverage = coverageMatrix(root, snapshot);
  ...
  const out = {
    ...
    uncovered_capabilities: coverage.uncovered,
    orphan_covers: coverage.orphan_covers,
    plan_gaps: planGaps,
    campaigns: ordered,
  };
  ...
  const lines = ['中期計画ビュー (--plan):', '', 'capability カバレッジ (covers 射影):'];
  if (coverage.capabilities.length === 0) {
    lines.push('  (vision に Capabilities 宣言なし)');
  }
  for (const c of coverage.capabilities) {
    if (c.covered) {
      const srcs = c.covered_by.map((s) => `${s.ref}${s.source === 'frontmatter' ? '(宣言)' : ''}`).join(', ');
      lines.push(`  [x] ${c.id}  <- ${srcs}`);
    } else {
      // 未カバー capability は空欄（カバー元なし）として可視化する。
      lines.push(`  [ ] ${c.id}  <- (未カバー)`);
    }
  }
  if (coverage.orphan_covers.length) {
    lines.push('  orphan covers (vision 未宣言の cap-id を covers):');
    for (const o of coverage.orphan_covers) {
      lines.push(`    ${o.ref} -> ${o.to}${o.source === 'frontmatter' ? '(宣言)' : ''}`);
    }
  }
  lines.push('', 'route=vision の計画在庫 gap:');
...
function cmdReport({ positional, root, jsonMode }) {
  ...
  const toolOwned = buildToolOwned(ref, kind, snapshot, events, root);
...
function buildToolOwned(ref, kind, snapshot, events, root) {
  const id = subjectId(ref);
  const owned = {
    source_refs: [ref],
    verification_refs: [],
  ...
  if (kind === 'campaign') {
    ...
    // completion 欄の coverage 概況（covers 射影）。この campaign の covers 元が
    // 被覆する capability と、全 capability の未カバー概況を機械記入する。
    if (root != null) {
      const matrix = coverageMatrix(root, snapshot);
      const coveredByThis = matrix.capabilities
        .filter((c) => c.covered_by.some((s) => s.ref === ref))
        .map((c) => c.id);
      owned.coverage_summary = {
        total: matrix.capabilities.length,
        covered: matrix.capabilities.filter((c) => c.covered).length,
        uncovered: matrix.uncovered,
        covered_by_this_campaign: coveredByThis,
      };
    }
  }
...
function renderReport(ref, kind, owned, docLang) {
  ...
  if (owned.coverage_summary) {
    const cs = owned.coverage_summary;
    parts.push(`coverage 概況:            ${cs.covered}/${cs.total} capability カバー済み`);
    parts.push(`  未カバー capability:    ${list(cs.uncovered)}`);
    parts.push(`  本 campaign のカバー先: ${list(cs.covered_by_this_campaign)}`);
  }
  parts.push('```\n');
