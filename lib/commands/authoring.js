...
import { normalizeRefs, refsByRel } from '../refs.js';
import { parseChecks } from './run.js';
...
// refs 正規化（normalizeRefs）と rel 抽出（refsByRel）は lib/refs.js に一本化した。
// doctor と同一実装を共有し、単一キー map（- upstream: vision:v001）の扱いを揃える。
...
function cmdNew({ positional, root }) {
  ...
  return { ok: true, command: `new ${kind}`, kind, id, slug, path: relFile, subject };
}
...
function cmdSpecReady({ positional, root }) {
  ...
  return { ok: true, command: 'spec ready', subject: specRef, to: 'ready', eventId: res.eventId };
}
...
function cmdCampaignLaunch({ positional, root }) {
  ...
  return { ok: true, command: 'campaign launch', subject: campRef, to: 'active', eventId: res.eventId, specs: advanced };
}
...
// charter の Cross-Checks 節から check 群を抽出する（spec の Checks と同一記法）。
// parseChecks の第 2 引数で節名を 'Cross-Checks' に切り替えて再利用する。
function parseCrossChecks(charterBody) {
  return parseChecks(charterBody, 'Cross-Checks');
}
...
function campaignCloseGuards(root, campId, snapshot) {
  ...
  // charter の Cross-Checks を CLI 子プロセス実行するガード群（04/13 の close 条件）。
  // spec の Checks と同一記法をパースし、verify.js の実行ロジック（runCheck）で
  // exit code を expect と照合する。1 件でも fail なら close は refuse（Default-FAIL）。
  // 各 check を独立ガードにすることで、attempt が guard_results へ check ごとの
  // 判定（name=check id, pass, detail に exit/expect）を焼き込む。cross-checks 節が
  // 無い/空なら追加ガードは 0 件で、従来どおり close 可（vacuous pass）。
  const crossChecks = body != null ? parseCrossChecks(body) : [];
  const crossCheckGuards = crossChecks.map((check) => {
    const fn = () => {
      const res = runCheck(root, check);
      return {
        name: `cross-check:${check.id}`,
        pass: res.pass,
        detail: res.pass
          ? `pass（exit=${res.exit}, expect=${res.expect}, run="${check.run}"）`
          : `fail（exit=${res.exit}, expect=${res.expect}, run="${check.run}"）`,
      };
    };
    // ガード名がスタックトレースや例外時 fallback で使えるよう関数名を付す。
    Object.defineProperty(fn, 'name', { value: `crossCheck_${check.id}` });
    return fn;
  });
  ...
  return [
    ...
    // charter の Cross-Checks を CLI 実行するガード群を末尾に連ねる（04/13 の close 条件）。
    ...crossCheckGuards,
  ];
}
...
function cmdCampaignClose({ positional, root }) {
  ...
  return { ok: true, command: 'campaign close', subject: campRef, to: 'closed', eventId: res.eventId, specs: advanced };
}
...
function nextForClose(campId, failed) {
  ...
  if (names.some((n) => n.startsWith('cross-check:'))) {
    cmds.push(`$EDITOR campaigns/${campId}-*/charter.md  # Cross-Checks が fail。実装を直すか check を修正`);
  }
  return cmds;
}
...
function cmdRetire({ positional, root }) {
  ...
  return { ok: true, command: 'retire', subject: ref, from, to: 'retired', eventId: res.eventId };
}
