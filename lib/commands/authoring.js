...
function normalizeRefs(raw) {
  ...
  if (Array.isArray(raw)) {
    for (const r of raw) {
      ...
      } else if (r && typeof r === 'object' && r.to) {
...
function cmdCampaignLaunch({ positional, root }) {
  ...
  return { ok: true, subject: campRef, to: 'active', eventId: res.eventId };
}
  ...
  return [
    ...
    function allTasksDone() {
      ...
      for (const [, r] of runsForCamp) {
        ...
        const t = r.task;
        if (Array.isArray(t)) t.forEach((x) => consumed.add(String(x)));
        else if (t != null) consumed.add(String(t));
      }
    ...
    function completionReport() {
      const reportRel = path.join('campaigns', findDirBySeq(root, 'campaigns', campId) || `${campId}`, 'report.md');
      ...
      return exists
        ...
        : { name: 'completion-report', pass: false, detail: `completion report がありません: ${reportRel}` };
    },
  ];
}
...
function cmdCampaignClose({ positional, root }) {
  ...
  return { ok: true, subject: campRef, to: 'closed', eventId: res.eventId };
}
