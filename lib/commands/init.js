...
import { initProjectContext, autoCommit, isGitRepo, baseCommit, isDirty } from '../gitops.js';
...
export function run({ positional, flags, jsonMode }) {
  ...
  // cc-iasd パッケージ自身（またはその clone）への init を拒否する。
  // 出荷資産（roles/ 等）を上書きし、監査 auto-commit でコード履歴を汚染するため。
  const targetPkg = path.join(target, 'package.json');
  if (fs.existsSync(targetPkg)) {
    let pkgName = null;
    try {
      pkgName = JSON.parse(fs.readFileSync(targetPkg, 'utf8')).name;
    } catch {
      pkgName = null;
    }
    if (pkgName === 'cc-iasd') {
      throw refuse(
        'init',
        [{ input: 'target', detail: `${target} は cc-iasd パッケージ自身です。project-context は別ディレクトリに作成してください` }],
        ['cc-iasd init <別ディレクトリ>'],
      );
    }
  }
  ...
  // 導入時 baseline イベント（14 5.4）。登録各 repo の導入時点 HEAD / dirty を刻む。
  // baseline は run 単位 base commit の起点であり、doctor の adoption-baseline 検査の入力になる。
  // 登録 repo が 0 件でも event は必ず 1 件刻む（data.repos = []）。
  const baselineRepos = repos.map((r) => {
    const abs = path.resolve(target, r.path);
    // git repo でない / HEAD 取得不能なら head=null / dirty=null。
    if (!isGitRepo(abs)) {
      return { name: r.name, head: null, dirty: null };
    }
    let head = null;
    let dirty = null;
    try {
      ...
      // HEAD 未成立（commit ゼロの git repo 等）なら head/dirty を null に落とす。
      head = null;
      dirty = null;
    }
    return { name: r.name, head, dirty };
  });
  append(target, {
    type: 'baseline.recorded',
    subject: 'project:root',
    actor: { kind: 'cli' },
    data: { repos: baselineRepos },
  });
