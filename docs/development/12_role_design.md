...
6. campaign creation and Devil's Advocate Design Launch Review
7. run creation
8. worker implementation and Planning Lead-launched code quality review
9. planning lead run reconciliation
10. Devil's Advocate Campaign Completion Review
11. compliance review after Devil's Advocate
12. completion report or escalation
13. lifecycle maintenance: log / reference / archive / outdate
```
...
Devil's Advocate は単一 role のまま維持し、起動 mode を分ける。Design Launch Review は spec design review と campaign planning の後、最初の run の前に campaign 走行可否を疑う。Campaign Completion Review は implementation と task-unit review の後、campaign 完了扱いの前に完了可否を疑う。どちらの mode でも Devil's Advocate Focus は監査範囲を制限しない。
...
sequenceDiagram
  ...
  PL->>DA: invoke Devils Advocate in Design Launch Review mode
  ...
  CLI-->>Evidence: reviews/review_timestamp_summary.md
  CLI-->>Evidence: logs/log_timestamp_review-add.md
  DA-->>PL: campaign launch findings
  alt campaign launch accepted
    ...
    PL->>CLI: cc-iasd escalate cNNN-campaign-id
    CLI-->>Evidence: reports/escalation_timestamp_cNNN-campaign-id.md
    CLI-->>Evidence: logs/log_timestamp_escalate.md
    PL->>H: escalation or upstream backtrack packet
    H-->>PL: human decision or correction direction
  end
  ...
  Note over PL,DA: Devil's Advocate Campaign Completion Review is a campaign completion condition
  PL->>DA: invoke Devils Advocate in Campaign Completion Review mode
...
- Devil's Advocate Design Launch Review は campaign 走行可否判断として扱い、最初の run の前に Planning Lead が呼び出す
- Devil's Advocate Campaign Completion Review は campaign completion condition として扱い、campaign 完了扱いの前に Planning Lead が呼び出す
