...
| `cc-iasd planning-feedback add <id>` | Execution Manager | `ops/planning-feedback/<feedback-id>.md`, create log |
| `cc-iasd planning-feedback resolve <id>` | Planning Lead | update planning feedback metadata, move to `ops/planning-feedback/archived/<feedback-id>.md`, create log |
| `cc-iasd planning-feedback view <id>` | Planning Lead | stdout only; active or archived planning feedback packet |
...
execution entry:
  ...
  Execution Manager -> Execution Handoff Packet / cc-iasd planning-feedback add
```
...
Planning Feedback Packet は execution entry から planning entry へ戻る標準 handoff であり、`cc-iasd planning-feedback add` で `ops/planning-feedback/` に作成する。Completion Report は evidence layer に残り、Planning Feedback Packet はそれを根拠に planning artifact へ戻すべき項目だけを分類する。Planning Lead が処理した後は `cc-iasd planning-feedback resolve` で `absorbed / rejected / deferred` のいずれかへ閉じる。
...
- planning-layer follow-up を `cc-iasd planning-feedback add` で Planning Feedback Packet として作成する
```
...
sequenceDiagram
  ...
  alt campaign completion accepted
    ...
    EM->>CLI: cc-iasd planning-feedback add pfNNN-feedback-id
    CLI-->>Ops: ops/planning-feedback/pfNNN-feedback-id.md
    EM-->>PL: Execution Handoff Packet and Planning Feedback Packet ID
    ...
    EM->>CLI: cc-iasd planning-feedback add pfNNN-feedback-id
    CLI-->>Ops: ops/planning-feedback/pfNNN-feedback-id.md
    EM-->>PL: Planning Feedback Packet ID when planning artifacts must change
  end
