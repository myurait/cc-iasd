...
AI が判断してよい品質観点は、記述の存在、参照の追跡可能性、矛盾、TBD / unresolved の残存、scope / non-goal / deferred / blocker の分離、priority / impact / verification の追跡可能性、次 role が推測なしに作業できるかである。

AI が判断してはならない品質観点は、product value の優先順位、ユーザー体験上の好み、外部 service 採用、infrastructure / cost / security / privacy / data retention、非目標の解除、feature priority 変更、roadmap 到達点変更である。これらは不足または human decision required として扱う。

Backtrack Request は次を含む。
