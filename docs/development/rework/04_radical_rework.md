...
作成日: 2026-07-03  
状態: 承認確定版 v0.3（全論点確定。本編反映と P1 実装計画へ進行可能）
...
```text
- 未確定事項は gap として登録する
- spec 本文中の未確定箇所は [UNRESOLVED: g-012] の形で gap を参照する
...
#   spec 本文には [UNRESOLVED: g-001] を記載
cc-iasd spec ready s001    # => 拒否: blocking gap g-001 が open。decide を促す
cc-iasd decide d002-csv-encoding             # 「BOM 付き UTF-8」を決裁。g-001 close
...
B1 clarify 統合         -> gap 台帳 + [UNRESOLVED: g-NNN] 参照の単一規約（6 章）
