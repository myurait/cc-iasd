...
- user/ の最小構造
- product/ の最小構造
- product/ideal/ の最小ファイル
- product/specs/ を Spec Kit 生成に完全委譲するか
- ops/scopes/ の最小構造
- ops/cycles/ の最小構造
- ops/evidence/ の最小構造
- src/ の初期状態
...
### 2.5 scope / cycle と spec の対応

```text
論点:
- 1 spec = 1 cycle か
- 1 spec に複数 cycle を許すか
- milestone は roadmap 上の到達点に留めるか
- cycle がどの scope refs を必須にするか
...
- lock.json の schema
- product/ideal/ の artifact schema
- product/specs/ の Spec Kit 委譲範囲
- ops/scopes/roadmaps/ の保存形式
- ops/scopes/features/ の item schema
- ops/scopes/milestones/ の保存形式
- ops/cycles/ の state / handoff / knowledge schema
- ops/evidence/logs/ の entry schema
- ops/evidence/reviews/ の review lifecycle
- ops/evidence/reports/ の report lifecycle
- milestone id の命名規則
- spec id と milestone id の対応
- tasks.md のどの粒度を実行単位にするか
- escalation report の scope ref
- completion report の scope ref
- user decisions と cycle-local 軽微判断の分離形式
- src/ の Git 管理方式
- cycle handoff packet の schema
