...
6. Execution Manager は `cc-iasd planning-feedback add` で Planning Feedback Packet を作成し、planning artifact を直接編集しない。
7. Planning Lead は Planning Feedback Packet を入力として別 entry point で再開し、必要な planning role へ分類し、処理後に `cc-iasd planning-feedback resolve` で閉じる。
...
- `cc-iasd planning-feedback add` で Planning Feedback Packet を作成する。
...
- Planning Feedback Packet の処理結果を `absorbed / rejected / deferred` のいずれかとして判断し、`cc-iasd planning-feedback resolve` で閉じる。
