...
- src/ 配下へ ledger 管理 artifact を持ち込まない
```
...
Spec Kit は、ledger の spec-driven development 互換性における主要な標準化参照元である。
...
ledger は Spec Kit の成果物正本性を採用しない。Spec Kit tooling は通常、実装対象 repository 側に `.specify/` や `specs/` を作り、Git branch から active feature を判定する。この前提は、`src/` 配下を cc-iasd 管理 artifact で汚染しないという ledger の絶対制約と衝突する。
...
ledger は実行 runtime を project-context 全体の所有者にはしない。

```text
ledger と実行 runtime の関係:
...
- src/ 配下に ledger 管理の仕様、runtime、evidence を置く
```
...
Spec Kit は実装対象 repository 内の `specs/<feature>/` を前提にする。ledger は `product/specs/<spec-id>/` を前提にする。

完全互換を目指すと、`src/` 配下に Spec Kit artifact を置く必要が出る。これは src isolation に反するため採用しない。

### 13.2 branch-based active spec

Spec Kit は Git branch から active feature を検出する。ledger は Git 管理を責務にしない。

そのため、ledger では active spec を branch ではなく scope / cycle / explicit ref で決める必要がある。

### 13.3 constitution ownership

Spec Kit の constitution は project-level rule である。ledger には `rules/` と project-policies がある。

両方を正本にすると制約が二重化する。ledger では `rules/` を正本にし、Spec Kit 互換 layer が必要な場合はそこから投影する。

### 13.4 plan.md

`plan.md` は Spec Kit 標準 artifact だが、ledger には roadmap、milestone、cycle handoff がある。
...
ledger では completion report、review、evidence がその責務を担うため、初期 artifact としては作らない。

### 13.6 source tree vocabulary

Spec Kit の plan template における `src/` は repository root 内の source code directory を意味する。一方 ledger の `src/` は成果物 project container である。

この語義差は互換性の最大リスクである。ledger の template は、source target を `src/<repo-id>/` として明示し、Spec Kit 標準の source tree 例をそのまま持ち込まない。

### 13.7 generated script / command lifecycle

Spec Kit は command と template が artifact を生成する。ledger がこれを再現すると再実装になる。

ledger は command lifecycle の完全再現ではなく、artifact schema、参照形式、cycle 接続だけを互換対象にする。
