...
- campaign / run 自走と evidence を product/specs/ の外側に重ねる
...
- cc-iasd: project-context、campaign / run、自走境界、evidence、escalation
...
| tasks | product/specs/ | run / runtime へ接続 |
| ideal | product/ideal/ | user/ 入力を正規化した product 正本 |
| features | ops/scopes/features/ | ideal と roadmap の間の scope layer |
| roadmap | ops/scopes/roadmaps/ | ideal から campaign / run への計画 artifact |
| implementation loop | 実行 runtime | 委譲・結果集約 |
| role / SOP | rules/ + BMAD / MetaGPT 参照 | 最小定義 |
| campaign | ops/execution/campaigns/ | task selector / stop condition / progression condition を持つ上位実行計画 |
| run autonomy | ops/execution/runs/ | 自走実行単位 |
...
- cc-iasd は project-context ownership、src isolation、run autonomy、escalation、evidence bridge に集中する
```
...
そのため、cc-iasd では active spec を branch ではなく scope / run / explicit ref で決める必要がある。
...
`plan.md` は Spec Kit 標準 artifact だが、cc-iasd には roadmap、campaign、run handoff がある。

`plan.md` を維持する場合は、implementation plan に限定し、roadmap / campaign / run state を含めない。衝突が大きい場合は別名を検討する。
...
cc-iasd は command lifecycle の完全再現ではなく、artifact schema、参照形式、run 接続だけを互換対象にする。
