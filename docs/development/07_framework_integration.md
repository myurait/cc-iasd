...
作成日: 2026-07-05  
状態: kernel 正本 v1.1（2026-07-09 更新。8 章にハーネス内蔵オーケストレーションとの棲み分けを追加）
...
## 8. ハーネス内蔵オーケストレーションとの棲み分け

Claude Code の Workflow（dynamic workflow）に代表される、ハーネス内蔵のマルチエージェントオーケストレーション機能は、cc-iasd と設計思想の根を共有する。「制御フローはコードで決定論的に固定し、LLM は葉の作業だけを行う」という原則は両者に共通であり、並列分解・構造化出力・段階構成という形も相似する。この近接を正しく認識した上で、関係を次のように定める。

### 8.1 競合する帯域と、しない帯域

単一セッションに収まる規模の作業では両者は正面から競合し、cc-iasd は選ばれない。ハーネス内蔵機能はスクリプト 1 本で即座に並列実行でき、cc-iasd は adhoc run でも init・open・verify の手順を要求するためである。この帯域で張り合うことはしない。

一方、次の 3 点はハーネス内蔵オーケストレーションが構造的に持たない性質であり、cc-iasd の存在理由である。

```text
- 寿命: 内蔵オーケストレーションは 1 セッション内で生まれて死ぬ（再開機構があっても
  同一セッション限定）。cc-iasd の journal は append-only の正本としてセッション・
  ツール・担当者を跨いで生き続ける。数十人月帯域ではこの差が全てである
- 証拠の信頼境界: 内蔵オーケストレーションはエージェントの最終報告（自己申告）を
  信じる。cc-iasd は自己申告の拒否が核であり、検証は CLI 子プロセス実行の実測のみ。
  エージェントが「clean である」と誤報告する失敗類型は実在し、実測でしか捕捉できない
- 人間の位置: 内蔵オーケストレーションに実行途中の人間ゲートはない。cc-iasd は
  decide の人間専権・escalation packet・gap 台帳を一級市民として構造化する
```

### 8.2 関係の定義: 実行エンジンとガバナンスカーネル

正しい関係は競合ではなくレイヤリングである。ハーネス内蔵オーケストレーション = 実行エンジン、cc-iasd = その上のガバナンスカーネル、と位置づける。cc-iasd は runtime 非依存であり、1 つの run の中身をハーネスのオーケストレーション機能が実行する構図は自然に成立する（将来の runtime adapter 候補。09 参照）。逆方向、つまり内蔵オーケストレーションが cc-iasd を包含する構図は、journal・人間ゲート・寿命を持たないため成立しない。

### 8.3 統合方針

ハーネスベンダーはオーケストレーション機能を吸収し続けるため、競合帯域は今後広がる。cc-iasd が守る芯は、ベンダーが利便機能として作りにくい性質に置く。

```text
- オーケストレーションの使い勝手（並列 fan-out・スケジューリング）では競わない。
  ハーネスの進化を実行エンジンとして consume する
- 守る芯: ベンダー中立な監査証跡 / 実測ベースの evidence / 人間判断の来歴
  （actor=human 刻印）/ 推測補完を構造で殺す上流チェーン
- 内蔵オーケストレーションの実装から取り込める機構（再開の prefix キャッシュ、
  構造化出力の強制、失敗の型付け等）は、この芯を強化する形で吸収する
```
