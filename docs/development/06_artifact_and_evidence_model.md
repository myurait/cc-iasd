...
Completion Report は planning artifact を直接更新しない。Execution Manager は Completion Report と同時に Planning Feedback Packet を作成できる。Planning Feedback Packet は、Execution Manager から planning entry point へ渡す正式 artifact であり、`ops/planning-feedback/` に置く。
...
`ops/planning-feedback/` にある packet は、planning re-entry で処理すべき active handoff である。処理後は `cc-iasd planning-feedback resolve` により status を `absorbed / rejected / deferred` のいずれかに更新し、`archived/` へ移動する。`routed` は resolution にしない。role や human へ渡しただけでは、まだ planning feedback が閉じたとは限らないためである。
