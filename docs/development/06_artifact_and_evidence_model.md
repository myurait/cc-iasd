...
`plan.md` は当面維持する。ただし、roadmap、milestone、cycle handoff と混同しないよう、implementation plan に限定する。名称は未決定事項として残し、実装前にリネームを検討できる状態にする。
...
`plan.md` は実装方針を記述する。feature roadmap、milestone queue、cycle state、handoff は含めない。
...
milestone file の保存形式は単一 Markdown file とする。

```text
milestone:
- Milestone ID
- Summary
- Status: ready-for-cycle / in-cycle / completed / blocked / archived
- Linked Feature
- Linked Roadmap
- Linked Spec
- Linked Tasks
...
milestone ID は lowercase kebab-case ASCII とする。spec ID も同じ規則を使う。対応関係は ID の一致に依存せず、`Linked Spec` と `Linked Tasks` の明示参照で表現する。
...
spec-driven development 的には、実装進行の中心は spec / task である。cycle は、それらを cc-iasd の project-context から安全に実行 runtime へ渡し、状態を追跡する transaction artifact である。
...
cycle state の result / status は次を使う。
...
tasks.md の実行単位は checklist item とする。cycle は単一 task、複数 task、または milestone linked tasks 全体を参照できるが、handoff では実行対象 task を明示する。
...
user decisions と cycle-local 軽微判断は分離する。人間による product / policy / scope 判断は `user/decisions.md` に置く。Planning Lead の軽微判断は発生した cycle state、knowledge、review、report のいずれかに閉じる。
...
```text
scope ref:
- ideal: product/ideal/<ideal-id>.md
- feature: ops/scopes/features/<feature-id>.md
- roadmap: ops/scopes/roadmaps/<roadmap-id>.md
- milestone: ops/scopes/milestones/<milestone-id>.md
- spec: product/specs/<spec-id>/spec.md
- tasks: product/specs/<spec-id>/tasks.md
- cycle: ops/cycles/<cycle-id>/state.md
