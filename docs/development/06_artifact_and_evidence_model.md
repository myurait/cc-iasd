...
session.started.data:
  { runtime, bundleDir, repos, inputHashes }
    ...
    inputHashes compile 安定入力の正規化ハッシュ（後述）。start は監査用途で記録する

session.resumed.data:
  { runtime, bundleDir, resumeBrief, priorSession, diff, inputHashes, compileReused, changedInputs }
    ...
    inputHashes    今回 resume の compile 安定入力ハッシュ（後述）
    compileReused  compile をスキップし既存 bundle を再利用したか（bool）
    changedInputs  直近 resume からの変化入力名（[] なら不変。初回は '(no-prior-hashes)'）
...
inputHashes は compile 出力（bundle.md、および claude-code adapter の settings/）を決定づける「安定入力」の正規化ハッシュ集合であり、`{ handoff, roleCard, reposBase, config }` を持つ。handoff / roleCard は content-hash（frontmatter 除外 + 空白正規化）、reposBase は key ソート後 JSON の sha256、config は `{ docLang, adapter }` の sha256 である。resume-brief は毎回再構成する動的入力のためハッシュ対象に含めない。上記はいずれも新 event 型の追加ではなく、既存 session.started / session.resumed の data 拡張である。resume のキャッシュ規則は 05 10 章を参照。

verify.recorded の data は次の確定形を持つ。

```text
verify.recorded.data:
  { pass, checks, failureReasons, surface, mergeConflicts? }
    pass           verification の成否
    checks         各 check の { id, exit, expect, pass, reason }（reason は下記閉集合）
    failureReasons 失敗 check（pass=false）の reason ユニーク集合（全 pass なら []）
    surface        Surfaces 照合結果（{ offSurface, forbidden }）
    mergeConflicts worktree 隔離 run で merge dry-run が conflict した場合のみ（省略可）
```

checks[].reason と failureReasons は 4.1 の理由型（passed / unverified / refuted の閉集合）である。reason は closed set 型の追加ではなく、既存 verify.recorded event の data 拡張である。
...
```text
verdict:
- Checks の実行結果（各 check の id / exit code / 期待値との照合 / reason）
- checks 全体の failureReasons（失敗 check の reason ユニーク集合。全 pass なら []）
- Surfaces 照合の結果（forbid 該当・off-surface の列挙）
- 生出力の捕捉先（stdout / stderr / diff.patch）
- payload sha256（対象成果物の content-hash）
```

各 check の照合結果は、pass/fail の 2 値ではなく理由型（reason）を閉集合で持つ。verdict-level の failureReasons はその集約であり、fail の主因が「実行不能」か「内容不一致」かを事後に判別できる。

```text
check の reason（閉集合。pass の定義は従来どおり exit 一致かつ実行成功）:
- passed      期待 exit と一致し、内容も走行も期待どおり（reason=passed ⇔ pass=true）
- unverified  実行不能。子プロセスの spawn 失敗（ENOENT / timeout 等の環境要因）
- refuted     プロセスは走ったが期待 exit と不一致（内容要因）
```

unverified も refuted も pass ではない（Default-FAIL 不変）。証拠を作れなかった（unverified）ことと、証拠が期待を反証した（refuted）ことを区別することで、拒否メッセージ・verify --json・human 出力に失敗の型を露出できる（露出面は 5 章 / 08 5 章、accept 拒否メッセージの理由型付与は 08 3.13）。

verdict JSON は evidence の verifications 層に保存され、対応する verify.recorded event が journal に記録される（data 確定形は 3.2）。verification の成立（pass / fail）が accept ガードの入力になるが、そのガードの成立条件は 05 に置く。
