...
既存リポジトリ内のドキュメントはコードと同じく src 名前空間の中身であり、project-context へコピーしない。コピーは二重管理と即時陳腐化を招く。

spec 本文から既存リポジトリ内のドキュメントを参照するときは、Surfaces と同じ `src/<repo>/<path>` の論理パスで参照する。project-context への持ち込み領域（reference/ 等）は設けない。Surfaces と同一名前空間で決定論的に解決でき、持ち込み領域は二重管理と即時陳腐化を招くためである。
...
かつて本章に置いていた 3 件の未確定事項はいずれも解消済みである。

```text
- 導入時 baseline イベント:
    init が各登録 repo の HEAD / dirty を baseline.recorded event として journal に刻む（5.4 / 06 3.2）
- doctor の導入時検査:
    doctor の adoption-baseline 検査（baseline 欠落・導入時 dirty を warning に）で解消（08 3.3）
- 既存ドキュメントの参照形式:
    src/<repo>/<path> の論理パス参照に確定（5.5）
```
