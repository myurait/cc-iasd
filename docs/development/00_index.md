...
cc-iasd は project-context 全体を所有し、成果物 project を `src/` に隔離し、Spec Kit を spec-driven artifact vocabulary の参照元として扱う。実行 runtime は task implementation loop の委譲先であり、cc-iasd の固有価値は campaign / run による自走、escalation、evidence bridge、src isolation に集中する。
...
05_autonomy_protocol.md
  campaign / run 自走、Planning Lead / Execution Manager の entry point 境界、停止条件、エスカレーション条件
...
13_simulation_scenarios.md
  cc-iasd の artifact 境界、campaign/run 粒度、debt routing を検証する再利用可能なシミュレーションシナリオ

rework/
  先行事例調査に基づく設計リワークの検討メモ。調査正本と採用検討記録を置く。
  検討メモであり設計正本ではない。採用確定分は本編ドキュメントへ反映する。
```
...
campaign / run 自走・エスカレーション・証跡索引を独自に提供する、
...
- ops/scopes/ で features / roadmaps を扱える
- ops/execution/ で campaigns / runs を扱える
