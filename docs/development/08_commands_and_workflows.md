...
cc-iasd run cleanup <run-id> [--force] | --stale [--days N]   # worktree 明示回収（agent-facing）
...
--worktree は対象 nested git repo ごとに base から隔離ブランチ ccisad/<run-id> を切り worktree を out/<run-id>/wt/<repo> に張る。spike は src 不変のため --worktree を無視する。worktree の既定（baseRef / cleanup / include / force / stale_days）は cc-iasd.yaml の worktree セクション（03 3.6）で宣言し、run open の CLI フラグ（--worktree / --isolate、および --force）がこれを上書きする。include が空でなければ列挙 repo のみ隔離する。handoff の合成元と合成失敗時の扱いは 06、上流欠落の判定・worktree 隔離の排他関係・後始末の統一規則は 05 を参照。
...
accept が verification pass=false で封鎖される場合、拒否メッセージは直近 verify.recorded の failureReasons を引いて理由型（reasons=refuted / reasons=unverified 等）を detail に付す（06 4.1）。
...
### 3.20 cc-iasd run cleanup

```text
目的: worktree 隔離 run の隔離作業ツリーを明示的に回収する（agent-facing）
入力: <run-id> [--force] | --stale [--days N]
出力: 回収結果（removed / kept と kept の理由）
遷移: 起こさない（out/ 配下の worktree 削除と note.appended {kind:'worktree-cleanup'} のみ）
```

run cleanup は 2 モードを持つ。`<run-id>` 指定は当該 run の worktree を回収し、`--force`（config.worktree.force を上書き）で dirty worktree も強制削除する。`--stale` は全 run を走査し、隔離ブランチ先端が現 HEAD の祖先（base 取込済み）かつ最終イベントから `--days N`（既定 config.worktree.stale_days）を超過した worktree のみ回収する。回収判定は git 実測（git merge-base --is-ancestor / status --porcelain）のみで行い、LLM の自己申告を評価しない。dirty で force なしの worktree は残置し理由を記録する。

run cleanup は doctor に含めない。doctor は無副作用の検査コマンドであり、worktree 削除のような副作用を持たせないためである（3.3）。終端後の自動後始末は accept / block / escalate が行い（05 7 章）、run cleanup はその取りこぼしや明示回収のための agent-facing コマンドである。
...
agent-facing（人間は学習不要。知識は事前学習ではなく in-band で供給される —
  ...
  run return / run verify / run accept / block / escalate / run cleanup / review record /
...
verification 由来の拒否（accept の verification ガード不成立、run return の diff-snapshot 失敗）は、欠けた入力の detail に失敗の理由型（unverified / refuted）を露出する。「実行できなかった（unverified）」か「実行結果が期待を反証した（refuted）」かが拒否メッセージ・verify --json・human 出力の各面で判別できる。理由型の定義は 06 4.1 を参照。
