...
`cc-iasd index evidence` は主要 command から外す。横断 index は正本化しない。
...
MVP の初期実装では、`ops/cycles/cycle_<timestamp>_<scope>/state.md`、`handoff.md`、`knowledge.md` を作る。既存 cycle record は上書きしない。
...
MVP の初期実装では、新規 log file を作る。既存 log file への追記は行わない。`current.md` は作らない。
...
14. 古くなった product artifact は outdated/、ops artifact は archived/ に退避する
```
...
MVP では、cycle handoff は cycle-local な Markdown packet でよい。
