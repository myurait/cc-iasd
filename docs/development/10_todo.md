...
P2 以降のスコープ（vision / spec / campaign のノード化、review gates、covers 射影、Tier 1 adapter、session resume、worktree 隔離 adapter、guard_results 再計算検証の拡充）は P1 では着手しない。
...
## 4. Phase 1 の残作業（1-D / 1-E）

1-C までで本編ドキュメントの kernel 正本化を進めた。Phase 1 を締めるための残作業は次である。
...
```text
- standard_flow_overview.svg の再生成:
    1-B で standard_flow_overview.mmd を 3 ロール + kernel 前提へ再作成したが、
    svg はサンドボックス制約（Chromium 起動不可）で再生成できず旧版を削除した。
    Chromium 利用可能環境で mmd から svg を再生成する
```
