...
> **開発状態のお知らせ（2026-07）**: 本プロジェクトの積極的な機能開発は停止しています。経緯は末尾の「awslabs/aidlc-workflows と本プロジェクトの現状」を参照してください。カーネル自体は計画スコープを完了し、全テスト green の完結状態です。
...
## awslabs/aidlc-workflows と本プロジェクトの現状

本プロジェクトは [awslabs/aidlc-workflows](https://github.com/awslabs/aidlc-workflows)（AWS の AI-DLC）と正面から競合します。特に開発中の v2 は、cc-iasd と同じ結論に独立に到達しています。すなわち、append-only のイベントストアを正本として状態を導出すること、決定論的な帳簿管理をコードが所有し LLM は判断だけを所有すること、worktree 隔離による並列実行、セッション再開、特定ハーネスへの非依存。カーネルの設計思想はほぼ同型です。

2026-07 時点で違うのは次の箇所だけです。

```text
cc-iasd にあって aidlc に（まだ）無いもの:
- 検証（CLI 子プロセス実測、Default-FAIL）が唯一のブロッキングゲートであること。
  aidlc の検証センサーは実測まで到達しているが advisory に留まり、実効ゲートは
  ステージごとの人間承認である
- 人間判断の actor 刻印（decide は人間専権で actor=human を journal に刻む）
- multi-repo の v0 ネイティブ対応
- 推測補完禁止の構造化差し戻し（gap 台帳 / backtrack request / escalation packet が
  一級オブジェクト）

aidlc にあって cc-iasd に無いもの:
- 方法論コンテンツ（5 フェーズ 32 ステージ・ドメイン専門家エージェント群）
- 訂正を永続的な振る舞いルールへ変換する learning loop
- AWS の開発体制・配布力・エコシステム
```

そして後者の最後の 1 行が決定的です。前者の差分は設計の優劣ではなくロードマップの時差であり、個人開発が AWS の開発速度と配布力に伍して維持し続けられる差ではないと判断しました。**実質的に、これは敗北宣言です。** これから同種のハーネスの採用を検討する方には、まず aidlc-workflows を評価することを勧めます。

本リポジトリの積極的な機能開発はここで停止します。カーネルは計画スコープ（P1〜P4）を完了し、全テスト green の完結状態で残します。同じ設計に独立到達した大資本の実装が現れたことは、この設計が正しかったことの証明でもあります。ここで得た設計知見は、別プロジェクトへ派生させます。
